...
char* Keccak::getHash()
{
    ...
    for (unsigned int i = 0; i < hashLength; i++)
        for (unsigned int j = 0; j < 8; j++) // 64 bits => 8 bytes
        {
            ...
            result[idx++] = dec2hex[oneByte >> 4];
            result[idx++] = dec2hex[oneByte & 15];
        }
...
char* Keccak::operator()(const void* data, size_t numBytes)
{
    reset();
    add(data, numBytes);
    return getHash();
...
char* Keccak::operator()(const char* text)
{
    reset();
